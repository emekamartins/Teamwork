...
/* DO PAGINATION FOR VIEWING ARTICLES */
...
exports.createArticle = async (request, response) => {
  ...
  try {
    ...
    const now = new Date();
    const postData = await pool.query('INSERT INTO posts (post_type, post_created_on) VALUES($1, $2) returning *', [post_type, now])
...
exports.viewArticles = async (request, response) => {
  try {
    ...
    if (request.query.author) {
      ...
      if (!rows || rowCount === 0) {
        return response.status(404).send({ message: 'No articles found' });
      }
      const data = articleData(rows);
      return response.status(200).send({ rowCount, data });
    }
...
exports.deleteArticle = async (request, response) => {
  try {
    ...
    return response.status(500).send({ status: "error", error: error });
  }
};
