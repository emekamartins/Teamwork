...
exports.viewAllGif = async (request, response) => {
  ...
  try {
    if (request.query.author) {
      ...
      const { rows, rowCount } = await pool.query('SELECT gifs.gif_id as Id, gifs.gif_title as title, gifs.gif_link as url, gifs.created_on as createdOn, users.first_name as author_firstname, users.last_name as author_lastname FROM gifs LEFT JOIN users ON gifs.author = users.team_id WHERE (users.last_name || users.first_name) LIKE $1', [searchByAuthor]);
      if (!rows || rowCount === 0) {
        return response.status(400).send({ message: 'No gif found' });
      }
     
      return response.status(200).send({ rowCount, rows });
    }

    if (request.query.orderBy) {
      const part = request.query.orderBy.split(':');
      const columnOrder = `${part[0]} ${part[1].toUpperCase()}`;
      const { rows, rowCount } = await pool.query(`SELECT gifs.gif_id as Id, gifs.gif_title as title, gifs.gif_link as url, gifs.created_on as created_on, users.first_name as author_firstname, users.last_name as author_lastname FROM gifs LEFT JOIN users ON gifs.author = users.team_id ORDER BY ${columnOrder} LIMIT ${limit} OFFSET ${offset}`);
    ...
    const { rows, rowCount } = await pool.query(`SELECT gifs.gif_id as Id, gifs.gif_title as title, gifs.gif_link as url, gifs.created_on as created_on, users.first_name as author_firstname, users.last_name as author_lastname FROM gifs LEFT JOIN users ON gifs.author = users.team_id LIMIT ${limit} OFFSET ${offset}`);
    if (!rows || rowCount === 0) {
      return response.status(400).send({ error: 'Invalid request' });
    }

    
    return response.status(200).json({ rowCount, rows });
  } catch (error) {
...
exports.viewGif = async (request, response) => {
  ...
  try {
    ...
    if (request.query.orderBy) {
      const part = request.query.orderBy.split(':');
      const columnOrder = `${part[0]} ${part[1].toUpperCase()}`;
      const { rows, rowCount } = await pool.query(`SELECT gifs.gif_id as Id, gifs.title as title, gifs.image_url as url, gifs.created_on as created_on, users.first_name as author_firstname, users.last_name as author_lastname FROM gifs LEFT JOIN users ON gifs.author = users.team_id WHERE gifs.author = $1 ORDER BY ${columnOrder} LIMIT ${limit} OFFSET ${offset}`, [request.user.ID]);
    ...
    const { rows, rowCount } = await pool.query(`SELECT gifs.gif_id as Id, gifs.title as title, gifs.image_url as url, gifs.created_on as created_on, users.first_name as author_firstname, users.last_name as author_lastname FROM gifs LEFT JOIN users ON gifs.author = users.team_id WHERE gifs.author = $1 LIMIT ${limit} OFFSET ${offset}`, [request.user.ID]);
    if (!rows || rowCount === 0) {
      return response.status(400).send({ message: 'No gif found' });
    }
